...
[Fglob_t] = createFglob(GEN, Nelem,Fx,Area,LM,BBnodes,t(d),wf);
...
if newmark
    ...
    %initialization    
    AA=Mglob + gamma*h*C + h^2*beta*Kglob;
...
if newmark
    for d = 2:length(t)
        ...
        pr_vel = qdot(:,d-1)+(1-gamma)*h*qdot2(:,d-1);% + gamma*h*qdot2(:,d);
        pr_disp = q(:,d-1)+h*qdot(:,d-1)+h^2*(1/2-beta)*qdot2(:,d-1);%+h^2*beta*qdot2(:,d);

        AA = Mglob + gamma*h*C + h^2*beta*Kglob;
        BB=Fglob_t - C*pr_vel- Kglob*pr_disp;
        qdot2(:,d) = AA\BB; 

        q(:,d) = pr_disp+h^2*beta*qdot2(:,d);
        qdot(:,d) = pr_vel + gamma*h*qdot2(:,d);

        u(:,d)=[qdot(:,d);q(:,d)];
        [solution] = solutionRetriever(GEN, sizeM, d, length(t), u, solution);%[w,bx,by]
...
% Bm = [C, Kglob; -speye(sizeM,sizeM), sparse(sizeM,sizeM)];
    for d = 2:length(t)
        ...
        u(:,d) = timeIntegration(u, d, GEN, Mglob, Kglob, C, G, h, theta); %[w,bx,by,lambda]
...
%==========================================================================
Name = 'dynamicFEM';
animatorFunc(solution,length(t),h,pp,ee,tt,BBnodes, Name)
...
wInterpolated = F(xx,yy);

figure
plot3(pp(1,:)',pp(2,:)',w,'ro','MarkerFaceColor','r');
hold on;grid on;
hPlate2=surf(xx,yy,wInterpolated);
